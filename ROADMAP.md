...
- **Situational coverage-gap audit — absorption / hotel / retail / industrial (queued)** — Re-audited the 71-case situational catalog and found the depth-pass skew from item 99 hadn't closed on every axis: `absorption` is still the thinnest category (1 of 71 cases, vs. 21 for `deal-process`), `hotel`/`retail`/`industrial` remain the thinnest asset classes (1/2/3 vs. 11 for multifamily), and `development` the thinnest role (9 vs. 40 for acquisitions). 10 new phrasings targeting all three axes at once were added to `docs/interview-questions.md` as `GAP` entries (proposed ids: `industrial-absorption-cap-rate-divergence`, `retail-anchor-backfill-absorption`, `retail-cotenancy-domino-risk`, `hotel-new-supply-absorption-wave`, `industrial-spec-vs-bts-absorption`, `industrial-small-bay-vs-bulk-absorption`, `retail-power-center-preleasing-absorption`, `hotel-ramp-to-stabilization`, `multifamily-phasing-absorption-pace`, `portfolio-absorption-comp-radius`). Phrasings only — none implemented as `SituationalCase` files yet. While auditing, also found and corrected five previously-flagged `interview-questions.md` GAPs that had already shipped but were never marked mapped.
