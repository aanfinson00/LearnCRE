...
- **Situational coverage-gap audit — beginner / absorption / development / hotel-retail-industrial (queued)** — Re-audited the 71-case situational catalog by category, asset class, role, and difficulty. Every one of `docs/interview-questions.md`'s 10 previously-flagged GAPs turned out to already be shipped (capex-reserve-discipline, refi-cap-stress/refiStressTest, fund-vs-deal-irr-gap/feeDragOnIrr, capital-allocation-priority, the waterfall cluster, risk-adjusted-return-framework, dev-ltc-vs-ltv/constructionLoanSizing, leaseUpReserve, groundLeaseVsFee, walk-distressed-1) but were never marked mapped — corrected all 10. Fresh audit found `beginner` is now the single thinnest dimension in the catalog (4 of 71 cases, dwarfed by 33 intermediate / 34 advanced), on top of the already-known thin spots: `absorption` category (1 of 71), `hotel`/`retail`/`industrial` asset classes (1/2/3 vs. 11 for multifamily), and `development` role (9 vs. 40 for acquisitions). 10 new phrasings stacking multiple thin dimensions per question were added to `docs/interview-questions.md` as `GAP` entries (proposed ids: `ltc-vs-ltv-basics`, `leaseup-velocity-checkpoint`, `revpar-vs-comp-first-look`, `clear-height-comp-adjustment`, `tenant-concentration-portfolio-risk`, `phased-delivery-absorption`, `dev-exit-cap-sensitivity`, `fixed-vs-cpi-bumps`, `dscr-rollover-sensitivity`, `retail-co-tenancy-clause`). Phrasings only — none implemented as `SituationalCase` files yet.
...
10. ✅ Modeling test UX polish (full: ⌘↵ next-empty, ⌘D fill-from-left, sticky-left row labels, ⌘↑/⌘↓ formula history)
