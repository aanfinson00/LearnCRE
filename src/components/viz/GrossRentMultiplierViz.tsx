import { formatMultiple, formatUsd } from '../../math/rounding';
import { BarRow } from './_BarRow';

interface Props {
  purchasePrice: number;
  gpr: number;
}

export function GrossRentMultiplierViz({ purchasePrice, gpr }: Props) {
  const grm = purchasePrice / gpr;
  const max = purchasePrice;
  const gprPct = (gpr / max) * 100;

  return (
    <div className="rounded-lg border border-warm-line bg-warm-paper/30 p-4 space-y-3">
      <div className="text-xs font-medium uppercase tracking-widest text-warm-mute">
        Visualization
      </div>

      <div className="space-y-3 num font-mono text-xs">
        <BarRow
          label="Purchase price"
          width={100}
          value={formatUsd(purchasePrice)}
          tone="muted"
          labelWidthPx={110}
        />
        <BarRow
          label="GPR / yr"
          width={gprPct}
          value={formatUsd(gpr)}
          tone="good"
          labelWidthPx={110}
        />
      </div>

      <div className="space-y-1 border-t border-warm-line pt-2 font-mono text-[11px] num">
        <div className="flex items-baseline justify-between">
          <span className="text-warm-stone">GRM (Price ÷ GPR)</span>
          <span className="font-medium text-copper-deep">
            {formatMultiple(grm)}
          </span>
        </div>
        <div className="text-[10px] leading-snug text-warm-mute">
          Rough valuation shorthand — multiply the property's annual gross rent
          by the GRM to get back to price. Lower = cheaper, higher = richer.
        </div>
      </div>
    </div>
  );
}
